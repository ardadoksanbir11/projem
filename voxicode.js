...
const prefix = 'o!' //PREFİXİNİZİ GİRİNİZ.
...
client.on('message', async message => {
  ...
  fetch(Link).then(() => {
    ...
    ** Yazdığınız Site Başarıyla Uptime Sistemimize Eklendi. **
    `)
    .addField(prefix+'linkler','Komutunu Kullanarak Ekledigin Linkleri Görebilirsin!')//Melek#7700 Tarafından Kodlandı
    .setTimestamp()
...
** orioN Uptime  Şuanda  \`${db.get('Proje')}\` URL'yi Sorunsuz Bir Şekilde 7/24 Aktif Tutuyor. **

** orioN Uptime  Bu Linklerden Sadece \`${db.fetch(`Sahiplik_${message.author.id}`) || null}\` Tane Senin URl'ni Uptime ediyor!📣 **
`)
...
`)
  .addField('**  Uptime  - Komutları**',`
»  **o!ekle (Glitch Show Linki)** = Botunuzu 7/24 Aktif Tutar.
» **o!linkler** = 7/24 Tuttuğum linkleri gösterir.
»  **o!say** = Tüm Uptime edilmiş bot sayısını gösterir.
`)
  .addField('** Uptime Bot - Hakkında**',`
» **Prefixim** = **${prefix}**
»  [Destek Sunucu](https://discord.gg/AeC9NKjkeZ)
»  [Uptime Botu Ekle](https://discord.com/api/oauth2/authorize?client_id=854636197024235520&permissions=8&scope=bot)
» **NOT: Botu İster Sunucusunda İster Özelden Kullanabilirsiniz Tercih Sizin**`)
  ...
  message.channel.send(pxd)
    ...
    message.author.send(new Discord.MessageEmbed().setColor('PURPLE').setDescription(`** Normal Linklerin:** \n\n\``+Linkleri.join('\n')+`\``).setThumbnail(message.author.avatarURL))
    }
...
client.on('ready', () => {
client.user.setActivity(`orioN | UUR'un hayalleriyle`, { type: 'PLAYING' })
client.user.setStatus('online')
  
  //client.user.setStatus('online') -> çevrimiçi -> Melek#7700 Tarafından Kodlandı
  //client.user.setStatus('dnd') -> rahatsız etmeyin -> Melek#7700 Tarafından Kodlandı
})

client.on("message", async message => {

  if(!message.content.startsWith("eval")) return;
  if(!["İDNİ GİR"].includes(message.author.id)) return;
...
//Melek#7700 Tarafından Kodlandı